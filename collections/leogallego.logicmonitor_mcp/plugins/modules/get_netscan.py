#!/usr/bin/python
# -*- coding: utf-8 -*-
# Auto-generated by AnsibleClaw from MCP tool: get_netscan
# GNU General Public License v3.0+ (see COPYING or https://www.gnu.org/licenses/gpl-3.0.txt)

from __future__ import absolute_import, annotations, division, print_function

__metaclass__ = type


DOCUMENTATION = r"""
---
module: get_netscan
short_description: Get detailed information about a specific netscan by ID in LogicMonitor (LM) monitoring. 

**Returns:** Complete netscan details: name, description, scan method, schedule, target networks/IPs, credentials, filters (include/exclude rules), resource/device properties to apply, collector assignment, duplicate detection settings, last execution results. 

**When to use:**
- Review netscan configuration before running
- Troubleshoot why certain resource/device not discovered
- Check credentials and filters
- Verify resource/device properties applied to discovered resources/devices
- Understand duplicate detection logic


**Configuration details returned:** 
- **Targets:** IP ranges, subnets, or cloud filters (e.g., "192.168.1.0/24", "All EC2 with tag:Environment=prod") 
- **Schedule:** How often scan runs (hourly, daily, weekly, on-demand) 
- **Credentials:** Which properties used for authentication (ssh.user, snmp.community) 
- **Filters:** Include/exclude rules (e.g., "Exclude IPs ending in .1", "Only Linux servers") 
- **Device properties:** Auto-applied to discovered resource/device (e.g., location, environment tags) 
- **Duplicate handling:** How to handle resource/device found in multiple scans 

**Troubleshooting use cases:** 
- "Why resource/device not discovered?" → Check if IP in target range and not excluded by filters 
- "Wrong credentials?" → Verify credential properties configured in netscan 
- "resources/Devices missing properties?" → Check default properties applied by netscan 

**Workflow:** Use "list\_netscans" to find netscanId, then use this tool to review complete configuration. 

**Related tools:** "list\_netscans" (find netscan), "update\_netscan" (modify), "run\_netscan" (execute now).
description:
  - Get detailed information about a specific netscan by ID in LogicMonitor (LM) monitoring. 

**Returns:** Complete netscan details: name, description, scan method, schedule, target networks/IPs, credentials, filters (include/exclude rules), resource/device properties to apply, collector assignment, duplicate detection settings, last execution results. 

**When to use:**
- Review netscan configuration before running
- Troubleshoot why certain resource/device not discovered
- Check credentials and filters
- Verify resource/device properties applied to discovered resources/devices
- Understand duplicate detection logic


**Configuration details returned:** 
- **Targets:** IP ranges, subnets, or cloud filters (e.g., "192.168.1.0/24", "All EC2 with tag:Environment=prod") 
- **Schedule:** How often scan runs (hourly, daily, weekly, on-demand) 
- **Credentials:** Which properties used for authentication (ssh.user, snmp.community) 
- **Filters:** Include/exclude rules (e.g., "Exclude IPs ending in .1", "Only Linux servers") 
- **Device properties:** Auto-applied to discovered resource/device (e.g., location, environment tags) 
- **Duplicate handling:** How to handle resource/device found in multiple scans 

**Troubleshooting use cases:** 
- "Why resource/device not discovered?" → Check if IP in target range and not excluded by filters 
- "Wrong credentials?" → Verify credential properties configured in netscan 
- "resources/Devices missing properties?" → Check default properties applied by netscan 

**Workflow:** Use "list\_netscans" to find netscanId, then use this tool to review complete configuration. 

**Related tools:** "list\_netscans" (find netscan), "update\_netscan" (modify), "run\_netscan" (execute now).
  - This module wraps the MCP tool 'get_netscan'.
  - Requires the C(ansible.mcp) collection and an MCP server connection.
version_added: "1.0.0"
author:
  - Auto-generated by AnsibleClaw
options:
  netscanId:
    description:
      - The ID of the netscan to retrieve
    type: float
    required: true
  fields:
    description:
      - Comma-separated list of fields to include in response. Examples: "id,displayName,hostStatus" or use "*" for all fields. Omit this parameter to receive a curated set of commonly used fields.
    type: str
    required: false
seealso:
  - module: ansible.mcp.run_tool
    description: Generic MCP tool runner this module delegates to.
requirements:
  - ansible.mcp collection installed
  - MCP server accessible via ansible.mcp.mcp connection
"""

EXAMPLES = r"""
---
- name: Get detailed information about a specific netscan by ID in LogicMonitor (LM) monitoring. 

**Returns:** Complete netscan details: name, description, scan method, schedule, target networks/IPs, credentials, filters (include/exclude rules), resource/device properties to apply, collector assignment, duplicate detection settings, last execution results. 

**When to use:**
- Review netscan configuration before running
- Troubleshoot why certain resource/device not discovered
- Check credentials and filters
- Verify resource/device properties applied to discovered resources/devices
- Understand duplicate detection logic


**Configuration details returned:** 
- **Targets:** IP ranges, subnets, or cloud filters (e.g., "192.168.1.0/24", "All EC2 with tag:Environment=prod") 
- **Schedule:** How often scan runs (hourly, daily, weekly, on-demand) 
- **Credentials:** Which properties used for authentication (ssh.user, snmp.community) 
- **Filters:** Include/exclude rules (e.g., "Exclude IPs ending in .1", "Only Linux servers") 
- **Device properties:** Auto-applied to discovered resource/device (e.g., location, environment tags) 
- **Duplicate handling:** How to handle resource/device found in multiple scans 

**Troubleshooting use cases:** 
- "Why resource/device not discovered?" → Check if IP in target range and not excluded by filters 
- "Wrong credentials?" → Verify credential properties configured in netscan 
- "resources/Devices missing properties?" → Check default properties applied by netscan 

**Workflow:** Use "list\_netscans" to find netscanId, then use this tool to review complete configuration. 

**Related tools:** "list\_netscans" (find netscan), "update\_netscan" (modify), "run\_netscan" (execute now).
  hosts: mcp_servers
  connection: ansible.mcp.mcp
  gather_facts: false
  vars:
    ansible_mcp_server_name: "CHANGEME"
  tasks:
    - name: Get detailed information about a specific netscan by ID in LogicMonitor (LM) monitoring. 

**Returns:** Complete netscan details: name, description, scan method, schedule, target networks/IPs, credentials, filters (include/exclude rules), resource/device properties to apply, collector assignment, duplicate detection settings, last execution results. 

**When to use:**
- Review netscan configuration before running
- Troubleshoot why certain resource/device not discovered
- Check credentials and filters
- Verify resource/device properties applied to discovered resources/devices
- Understand duplicate detection logic


**Configuration details returned:** 
- **Targets:** IP ranges, subnets, or cloud filters (e.g., "192.168.1.0/24", "All EC2 with tag:Environment=prod") 
- **Schedule:** How often scan runs (hourly, daily, weekly, on-demand) 
- **Credentials:** Which properties used for authentication (ssh.user, snmp.community) 
- **Filters:** Include/exclude rules (e.g., "Exclude IPs ending in .1", "Only Linux servers") 
- **Device properties:** Auto-applied to discovered resource/device (e.g., location, environment tags) 
- **Duplicate handling:** How to handle resource/device found in multiple scans 

**Troubleshooting use cases:** 
- "Why resource/device not discovered?" → Check if IP in target range and not excluded by filters 
- "Wrong credentials?" → Verify credential properties configured in netscan 
- "resources/Devices missing properties?" → Check default properties applied by netscan 

**Workflow:** Use "list\_netscans" to find netscanId, then use this tool to review complete configuration. 

**Related tools:** "list\_netscans" (find netscan), "update\_netscan" (modify), "run\_netscan" (execute now).
      leogallego.logicmonitor_mcp.get_netscan:
        netscanId: "CHANGEME"
      register: result
"""

RETURN = r"""
---
content:
  description: List of content blocks returned by the MCP tool.
  type: list
  returned: always
  elements: dict
is_error:
  description: Whether the MCP tool reported an error.
  type: bool
  returned: always
structured_content:
  description: Optional structured result data from the MCP tool.
  type: dict
  returned: when available
tool_name:
  description: Name of the MCP tool that was called.
  type: str
  returned: always
tool_classification:
  description: Tool classification information (read_only, destructive).
  type: dict
  returned: always
"""


MCP_TOOL_NAME = "get_netscan"


from ansible.module_utils.basic import AnsibleModule


def main():
    module = AnsibleModule(
        argument_spec=dict(
            netscanId=dict(
                type="float",
                required=True,
            ),
            fields=dict(
                type="str",
            ),
        ),
        supports_check_mode=True,
    )

    if module.check_mode:
        module.exit_json(changed=False, msg="Check mode: no MCP call made")

    tool_args = {k: v for k, v in module.params.items() if v is not None}

    module.exit_json(
        changed=False,
        _mcp_delegate=dict(
            name=MCP_TOOL_NAME,
            args=tool_args,
        ),
    )


if __name__ == "__main__":
    main()
